...
* **Current stub:** Until the backend integration lands, the Next.js app skips auth enforcement and renders the console for every visitor; re-enable the guard once the token wiring is complete.
...
* **Threads index landing:** `/explore/threads` now lists only sync-enabled mailing lists (names, slugs, optional descriptions) and links straight into the thread browser for the chosen slug. Errors render a destructive alert; empty states call out when no lists are enabled so the UI doesn’t feel unfinished.
