...
export function SyncPanel() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(pollStatus, isActive ? 1000 : 5000);
  ...
  const loadMailingLists = async () => {
    try {
      const data = await apiClient.getMailingLists();
      setLists(data);
    } catch (err) {
  ...
  const handleToggle = async (slug: string, currentEnabled: boolean) => {
    try {
      await apiClient.toggleMailingList(slug, !currentEnabled);
  ...
  const handleStartSync = async () => {
    ...
    setError(null);
    try {
      await apiClient.queueSync(enabledSlugs);
    } catch (err) {
