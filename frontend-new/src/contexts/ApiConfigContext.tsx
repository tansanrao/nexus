...
interface ApiConfigContextType {
  ...
  setSelectedMailingList: (slug: string | null) => void;
}
...
const MAILING_LIST_KEY = 'nexus_selected_mailing_list';
...
export function ApiConfigProvider({ children }: { children: ReactNode }) {
  ...
  const [selectedMailingList, setSelectedMailingListState] = useState<string | null>(() => {
    return localStorage.getItem(MAILING_LIST_KEY);
  });
  ...
  const setSelectedMailingList = (slug: string | null) => {
    setSelectedMailingListState(slug);
    if (slug) {
      localStorage.setItem(MAILING_LIST_KEY, slug);
    } else {
      localStorage.removeItem(MAILING_LIST_KEY);
    }
  };
